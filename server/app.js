...
const options = {
  key: fs.readFileSync(
    "/etc/letsencrypt/live/chainlink.restapi.ca/privkey.pem"
  ),
...
app.post("/api/users", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [username.toLowerCase(), hashedPass]
    );
...
app.get("/api/users", verifyJwt, async (req, res) => {
  ...
  if (id === 33) {
    try {
      ...
      const placeholder = result.rows.map(
        ({
          id,
          username,
          links,
      ...
      return res
        .status(500)
        .send({ message: "Error getting all users from database" });
    }
  } else {
    return res.status(401).send({ message: "Not authorized" });
  }
...
app.get("/api/users/:username", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send({ message: "Error getting user from database" });
  }
});
...
app.delete("/api/users/", verifyJwt, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send({ message: "Error deleting user from database" });
  }
});
...
app.put("/api/users/", verifyJwt, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send({ message: "Could not update information on database" });
  }
});
...
app.put("/api/users/links", verifyJwt, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE users SET links = $1 WHERE id = $2 RETURNING *",
    ...
    return res
      .status(500)
      .send({ message: "Could not update links on database" });
  }
});
